...
public class NetEncoder extends MessageToMessageEncoder<IRequest>
{
	public static Charset charset = Charset.forName("US-ASCII");
	
	@Override
	protected void encode(ChannelHandlerContext ctx, IRequest msg, List<Object> out) throws Exception
	{
		int l = 1;
		ByteBuffer name = charset.encode(msg.name());
		l += name.capacity();
		ByteBuf dat = msg.data();
		l += dat.capacity();
